...
def svm_loss_vectorized(W, X, y, reg):
  ...
  loss = 0.0
  scores=X.mm(W)
  margin=scores-scores[range(num_train),y].reshape(-1,1)+1
...
def train_linear_classifier(loss_func, W, X, y, learning_rate=1e-3,
  ...
  for it in range(num_iters):
    indx=torch.randint(num_train,size=(1,1),device='cuda')
    X_batch =X[indx:indx+batch_size]
    print(X_batch.shape)
    y_batch = y[indx:indx+batch_size]
    #########################################################################
    # TODO:                                                                 #
    # Sample batch_size elements from the training data and their           #
    # corresponding labels to use in this round of gradient descent.        #
...
def svm_get_search_params():
  ...
  learning_rates = [0.001, 0.01,0.1,3*0.001] # learning rate candidates, e.g. [1e-3, 1e-2, ...]
  regularization_strengths = [1,10,0.1,0.01] # regularization strengths candidates e.g. [1e0, 1e1, ...]
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  data_dict['y_train'], learning_rate=lr, reg=reg, num_iters=num_iters,
            batch_size=200, verbose=False)
  y_train_pred = cls.predict(data_dict['X_train'])
  train_acc = 100*(y_train_pred == data_dict['y_train']).sum()/len(y_train_pred)

  y_val_pred = cls.predict(data_dict['X_val'])
  val_acc = 100*(y_val_pred == data_dict['y_val']).sum()/len(y_val_pred)
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores=W.t().mv(X[i])
    softmax=torch.exp(scores[y[i]])/torch.sum(torch.exp(scores))
    loss+=-1*torch.log(softmax)
     #for numerical stability:
    const=-torch.max(scores)
    softmax2=torch.exp(scores[y[i]]+const)/torch.sum(torch.exp(scores+const))

    loss2=-1*torch.log(softmax2)
    for j in range(num_classes):
      if j==y[i]:
        dW[:,j]+=softmax2*X[i]-X[i]
      else:
        dW[:,j]+=torch.exp(scores[j]+const)/torch.sum(torch.exp(scores+const))*X[i]
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  softmax[range(num_train),y]=(torch.exp(scores[range(num_train),y]))/torch.sum(torch.exp(scores),1)
  
  softmax2=torch.sum(softmax,1)
  
  loss=torch.sum(-1*torch.log(softmax2))
  
 
  softmaxall=torch.exp(scores)/torch.sum(torch.exp(scores),1 ,keepdim=True)
  softmaxall[range(num_train),y]-=1
  dscores=softmaxall
  dW=X.t()@dscores
...
def softmax_get_search_params():
  ...
  - regularization_strengths: regularization strengths candidates
                              e.g. [1e0, 1e1, ...]
  """
  learning_rates = [1000,100,10,1,1e-1,1e-2,1e-3,1e-4]
  regularization_strengths = [3e-4,3e-9]
