...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  for i in range(num_train):
    dists[i]=torch.sum((x_test.view(num_test,-1)-x_train[i].view(1,-1))**2,1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  term1 = -2*x_train.reshape(num_train,-1) @  x_test.reshape(num_test,-1).t()
 
  term2=torch.sum((x_train.reshape(num_train,-1)**2),1)
  
  term3=torch.sum((x_test.reshape(num_test,-1).t()**2),0)

  dists=term1+term2.reshape(-1,1)+term3.reshape(1,-1)
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  output,indx=torch.topk(dists,k,0,largest=False)
 
  y_pred=torch.mode(y_train[indx],dim=0).values
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  # Hint: torch.chunk                                                          #
  ##############################################################################
  chunkx=torch.chunk(x_train,num_folds)
  chunky=torch.chunk(y_train,num_folds)
  for i in range(num_folds):
    x_train_folds.append(chunkx[i])
    y_train_folds.append(chunky[i])
  ...
  for choice in k_choices:
      for i in range(num_folds):
        ...
        xtrain,ytrain=torch.cat((*x_train_folds[:i],*x_train_folds[i+1:])), torch.cat((*y_train_folds[:i],*y_train_folds[i+1:]))
        classifier = KnnClassifier(xtrain, ytrain)
        accuracy=classifier.check_accuracy(x_val, y_val, k=choice,quiet=True)
        
        if choice not in k_to_accuracies:
          k_to_accuracies[choice]=[accuracy]
        else:
          k_to_accuracies[choice].append(accuracy)
